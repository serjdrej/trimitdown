...
- Бинарники не подписаны сертификатом разработчика (ни Apple, ни Microsoft) — при первом
  запуске нужно одно подтверждение. На macOS это Системные настройки → «Конфиденциальность
  и безопасность» → «Всё равно открыть»; прежний путь «правый клик → Открыть» больше не
  работает. На Windows — окно SmartScreen: «Подробнее» → «Выполнить в любом случае».
