...
плюс локальный `tests/data/table_detection/labelset-files.json` (в `.gitignore`),
который переводит хеши в имена. Без переменной набор скипается целиком; при **неполном**
корпусе он не скипается, а падает — измерение на частичном наборе бессмысленно.
