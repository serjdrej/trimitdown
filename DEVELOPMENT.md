...
- `src/trimitdown/` — публикуемый CLI-пакет и чистая логика конвертации, не зависящая от
  FastAPI. `core/converter.py` — HTTP-слой и работа с архивом для приложений, вызывающие пакет.
  `server_app.py` и `docker-server/app.py` — тонкие обвязки над этим слоем (различаются только
  путями к архиву/статике и специфичными для площадки роутами: `/cert` и `?raw=` у
  docker-сервера, `/api/mode` у десктопа).
...
- `server_app.py` — FastAPI-роуты для локального (офлайн) режима десктоп-приложений; конвертация
  выполняется пакетом `trimitdown` через HTTP-слой.
...
```bash
pip install --force-reinstall ./packages/trimitdown-pdf
```

PyInstaller не умеет ходить за editable-установкой: он соберёт внешне исправный
...
### Два набора: переносимый и корпусный

`pytest -m "not corpus"` — то, что гоняет CI. Работает на синтетических PDF из
`tests/pdf_fixtures.py`, ничего внешнего не требует и **не должен давать ни одного
скипа**: молчаливый скип в зелёном прогоне здесь уже приводил к ложной уверенности,
поэтому CI на это отдельно проверяет.

`pytest -m corpus` — измерение детекции на реальных документах (45 настоящих таблиц,
14 рамок вёрстки, 15 мусора от диаграмм). Корпус — чужие копирайтные документы, его
нет в репозитории и не будет; имён файлов там тоже нет — разметка опознаёт документы
по хешу. Нужны:
...
плюс `labelset-files.json`, который переводит хеши в имена. Без переменной набор
скипается целиком; при **неполном** корпусе он не скипается, а падает — измерение на
частичном наборе бессмысленно.

**Это предрелизный гейт.** CI его прогнать не может, поэтому перед каждым релизом
`pytest -m corpus` запускается вручную на машине с корпусом. Зелёный CI сам по себе
не означает, что детекция не поехала.

## Приватные данные — вне дерева репозитория

Всё, что несёт имена файлов корпуса или иные ПДн, лежит **снаружи** репозитория, в
соседней папке `../trimitdown-private` (переопределяется переменной `TRIMITDOWN_PRIVATE`).
Так его физически нельзя закоммитить — нет правила в `.gitignore`, которое можно забыть,
и `git add -f` не за что зацепиться. Однажды опубликованный листинг корпуса стоил переезда
репозитория; хранение этих данных вне рабочего дерева — и есть исправление.

Туда пишут: `labelset-files.json` (хеш→имя), `measure-corpus-details.jsonl` (вывод
`scripts/measure_corpus.py --details`), `rfsweep.jsonl`, `q1q2.jsonl`, и `pii-patterns.txt`
(литералы для хука ниже). Код читает эту папку с фолбэком на старое место в дереве.

**Pre-commit guard.** В `.githooks/pre-commit` — сканер, блокирующий коммит, если в
staged-изменениях есть маркеры ПДн. Устанавливается один раз на клон:

```bash
git config core.hooksPath .githooks
```

Паттерны в самом хуке неидентифицирующие (типы документов, общий вид пути); конкретные
литералы (фамилия, почта, старые логин/хост) — в `../trimitdown-private/pii-patterns.txt`,
вне дерева. **Обхода нет** — `AGENTS.md` запрещает его прямым текстом. Ложное срабатывание
чинится переформулировкой строки; сужение паттерна требует согласия владельца.

## Пересборка `.app` вручную (если нужно проверить именно собранный бинарник)

```bash
pip install pyinstaller
pip install --force-reinstall ./packages/trimitdown-pdf   # снять editable, см. выше
python generate_iconset.py                                # iconset не в репо, он derived
iconutil -c icns mac-build/AppIcon.iconset -o mac-build/AppIcon.icns
pyinstaller main.spec
grep -c trimitdown_pdf build/main/Analysis-00.toc          # > 0 — движок в бандле
```
